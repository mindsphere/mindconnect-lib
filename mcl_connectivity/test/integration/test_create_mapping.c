/*!
 * @file     test_create_mapping.c
 * @brief    Integration tests for creating data point mapping.
 *
 ...
 */
void test_create_mapping_001(void)
{
    mcl_error_t code;
    ...
    const char *user_agent = "CONFIGURATION:SERVICE=ONBOARD;HTTP_STATUS=201;%"\
        ...
        "SERVICE=MAPPING;HTTP_STATUS=201;%";
    ...
    TEST_ASSERT_EQUAL_MESSAGE(MCL_OK, code, "Mapping initialization failed.");

    // Set mapping parameters.
    code = mcl_mapping_set_parameter(mapping, MCL_MAPPING_PARAMETER_DATA_POINT_ID, "data_point_id");
    TEST_ASSERT_EQUAL_MESSAGE(MCL_OK, code, "Setting data point id failed.");

    code = mcl_mapping_set_parameter(mapping, MCL_MAPPING_PARAMETER_ENTITY_ID, "entity_id");
    TEST_ASSERT_EQUAL_MESSAGE(MCL_OK, code, "Setting entity id failed.");

    code = mcl_mapping_set_parameter(mapping, MCL_MAPPING_PARAMETER_PROPERTY_SET_NAME, "property_set_name");
    TEST_ASSERT_EQUAL_MESSAGE(MCL_OK, code, "Setting property set name id failed.");

    code = mcl_mapping_set_parameter(mapping, MCL_MAPPING_PARAMETER_PROPERTY_NAME, "property name");
    TEST_ASSERT_EQUAL_MESSAGE(MCL_OK, code, "Setting property name failed.");

    code = mcl_connectivity_create_mapping(connectivity, mapping);
    TEST_ASSERT_EQUAL_MESSAGE(MCL_OK, code, "Creating mapping failed.");
...
/**
 * GIVEN : A valid mapping to be created and an expired access token.
 * WHEN  : connectivity_processor_create_mapping() is called and server returns 401.
 * THEN  : MCL_UNAUTHORIZED is returned.
 */
void test_create_mapping_002(void)
{
    mcl_error_t code;
    mcl_uint16_t mindsphere_port = 1080;
    mcl_uint16_t proxy_port = 8888;
    ...
    const char *user_agent = "CONFIGURATION:SERVICE=ONBOARD;HTTP_STATUS=201;%"\
        "SERVICE=ACCESS_TOKEN;HTTP_STATUS=200;%"\
        "SERVICE=KEY_ROTATION;HTTP_STATUS=200;%"\
        "SERVICE=MAPPING;HTTP_STATUS=401;%";
    const char *tenant = "br-smk1";
    const char *iat = "eyJ0eXAiOiJKV1QiLCJraWQiOiJrZXkxIiwiYWxnIjoiUlMyNTYifQ==..pNkVTP6IhBtUbFy0o4fdy5TCdYeoODFdbt298gawvt0=";
    E_MCL_PROXY proxy_type = MCL_PROXY_HTTP;
    E_MCL_SECURITY_PROFILE security_profile = MCL_SECURITY_RSA_3072;
    ...
    code = mcl_connectivity_create_mapping(connectivity, mapping);
    TEST_ASSERT_EQUAL_MESSAGE(MCL_UNAUTHORIZED, code, "MCL_UNAUTHORIZED should have been returned.");
