/*!
 * @file     store_upload.c
 * @brief    Example custom agent code which uploads store.
 *
 * It is assumed that data source configuration is already uploaded which complies with the timeseries items you want to exchange.
 *
...
int main(void)
{
    ...
    // File to exchange via store.
    mcl_file_t *file = MCL_NULL;

    // Timeseries to exchange via store.
    ...
    if (MCL_OK == code)
    {
        code = safe_exchange(connectivity, core, store);
        printf("Exchanging store : %s.\n", MCL_CONNECTIVITY_CODE_TO_STRING(code));
    }
