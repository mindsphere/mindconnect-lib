...
# set offsetIndex
    def setOffsetIndex(offset)    
        @offsetIndex = offset.to_i
    end
...
# Test was flagged as being ingored so format the output
    def testIgnored(array)
        itemIndex = array.index("IGNORE") + 1
        testName = array[itemIndex - 2]
        reason = array[itemIndex].chomp
        testSuiteVerify(array[@className])
        printf "%-40s IGNORED\n", testName
        if @xmlOut == true
            @arrayList.push "     <testcase classname=\"" + @testSuite + "\" name=\"" + testName + "\">"
...
# Test was flagged as having failed  so format the line
    def testFailed(array)
        itemIndex = array.index("FAIL") + 1
        testName = array[itemIndex - 2]
        reason = array[itemIndex]
        for i in itemIndex + 1..array.length - 1
            reason = reason + ":" + array[i]
        end
        reason = reason.chomp + " at line: " + array[itemIndex - 3]
        testSuiteVerify(array[@className])
        printf "%-40s FAILED\n", testName
        if @xmlOut == true
            @arrayList.push "     <testcase classname=\"" + @testSuite + "\" name=\"" + testName + "\">"
...
# Main function used to parse the file that was captured.
    def process(name)
        ...
        File.open(name, "rb").each do |line|
...
if ARGV.size >= 1 
    ARGV.each do |a|
        ...
        elsif a.include? "-offset-index"
            arr = a.split("=")
            parseMyFile.setOffsetIndex(arr[1])
        else
